...
def worker_manager(mode: str):
    ...
    if mode == "on":
        ...
        daemon_status     = mina.get_daemon_status()
        ...
        if status_worker_pub is None or int(status_worker_fee) != WORKER_FEE:
            ...
            worker_address = mina.get_daemon_status()["daemonStatus"]["snarkWorker"]
            write_to_log(f'Worker status check. worker_pub_key = {worker_address}')
            data = mina.set_current_snark_worker(WORKER_PUB_KEY, WORKER_FEE)

    elif mode == "off":
        write_to_log("Turn off worker")
        data = mina.set_current_snark_worker(None, 0)
        worker_address = mina.get_daemon_status()["daemonStatus"]["snarkWorker"]
        write_to_log(f'Worker status check. worker_pub_key = {worker_address}')
    return data
...
try:
    mina = Client(graphql_host=GRAPHQL_HOST, graphql_port=GRAPHQL_PORT)
    ...
    write_to_log(f'Can\'t connect to graphql {GRAPHQL_HOST}:{GRAPHQL_PORT}. '
                 f'Check error log', print_mode="warning")
    exit(1)
...
while True:
    try:
        ...
        write_to_log(f'Parse error: {parseErr}', print_mode="warning")
        time.sleep(10)
